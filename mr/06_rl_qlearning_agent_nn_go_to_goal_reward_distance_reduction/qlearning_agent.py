...
class DeepQLearningAgent:
    ...
    def get_state(self, goal_pos):
        ...
        return np.array([angle_sector, dist_level], dtype=np.float32)
...
class QLearningDemo(QWidget):
    ...
    def update_env(self):
        ...
        if dist < 15:
            reward = 100
            # Effizienz = Schritte / Startdistanz
    ...
    def paintEvent(self, event):
        ...
        if self.last_state is not None:
            ...
            x, y = self.agent.pos
            offset = 25
            angle_label = int(self.last_state[0] * (360 // NUM_ANGLE_SECTORS))
            qp.drawText(x + 30, y - 50, f"Zustand: Winkel={angle_label}°, Distanz-Level={int(self.last_state[1])}")
...
if __name__ == '__main__':
    ...
    demo = QLearningDemo()
    demo.show()
    plt.ion()  # interaktiver Modus für Live-Plot
    plt.show()
    sys.exit(app.exec())
